...
Кластер ElasticSearch состоит из 3 узлов. Каждый индекс имеет 16 шардов, имеющих первичную копию , а также две реплики, что обеспечивает сохранность и целостность данных в случае сбоев в работе host-сервера:
<div align="center">
...
| Сценарий отказа |Условие|  Влияние на работоспособность кластера | Работоспособность | Описание |
|------------------------|---|-------------------------------|---------------------|---------------------------|
| Отказ одного узла    | Остальные узлы работаю  |  Незначительное       | Да              | Потери данных не наблюдается |
| Отказ двух любых узлов   |  Оставшийся узел работает | Значительное       | Да              | Кластер доступен только для чтения данных, добавлять новые документы невозможно  |
| Отказ трех любых узлов   |   | Критическое      | Нет              | Кластер недоступен, необходимо ручное вмешательство для восстановления|


Рассмотрим случай, с включением узла кластера:
| Сценарий включения |Условие|  Влияние на данные кластера | Описание |
|------------------------|------------------------------|---------------------|---------------------------|  
| Включение неработающего узла   | Передать данные перед включением  |Незначительное  | Данные реплицируются, кластер работает на запись\чтение |
| Включение 2 неработающих узлов   | Передать данные перед включением  |Значительное  | Включеные узлы подключаются к кластеру. Переданные данные потеряны. Кластер восстановил возможность записывать данные|
